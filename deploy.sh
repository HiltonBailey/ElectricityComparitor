...
if [ "$HTTP_CODE" = "204" ] || [ "$HTTP_CODE" = "200" ]; then
    ...
    SEED_CODE=$(curl -s -m 10 -o /dev/null -w "%{http_code}" \
        ...
        --data-urlencode "headers=name,model,dsc,sub,off_pk,sh_pk,pk_pk,off_fit,sh_fit,pk_fit,sp_fit,sp_limit,off_s,off_e,pk_s,pk_e,sp_s,sp_e,exp_s,exp_e,exp_rate,ev_s,ev_e,ev_pk,off_limit" \
